...
function input_matpower_data(bus_data, branch_data, gen_data, gencost_data, baseMVA)
    ...
    read_tap_ratio(x) = abs(x) < 0.0001 ? 1 : x
    for i in 1:num_branch
        branches[i] = (i,branch_data.fbus[i], branch_data.tbus[i])
        tap_ratio[i] = read_tap_ratio(branch_data.ratio[i])
    ...
    gen = 1:num_gen
    ref_bus = findfirst(x->x==3, bus_data[!,:type])
    network_data = power_system_data(bus_data.bus_i,bus_data.Bs,bus_data.Gs,bus_data.Pd,bus_data.Qd,bus_data.Vmax, bus_data.Vmin,ref_bus,
                            branches,tap_ratio,branch_shunt_suscep,branch_shunt_conduc,phase_angle,branch_resist,branch_react,branch_current_limit,
                            gen,gen_data.bus,gen_data.Pmax,gen_data.Pmin, gen_data.Qmax, gen_data.Qmin, gencost_data.c0,gencost_data.c1,gencost_data.c2,baseMVA)
    return network_data
end