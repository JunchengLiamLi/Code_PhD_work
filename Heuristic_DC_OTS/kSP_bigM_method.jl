...
# compute k shortest paths between 2 ending nodes for each branch in a given data instance
...
function ksp_for_DC_OTS(bus_data, branch_data, gen_data, ksp_paths, max_k, max_check_edge)
    ...
    for i in branch_data.index
        ...
        while !k_found && j < max_k
            ...
            # select the edges in the path such that the graph is still connected after removing such an edge
            #-------------------------------------------------------------------------------------------------------
            # Construct the graph
            #-----------------------------------
            graph = nx.Graph()
            remaining_edges_idx = setdiff(branch_data.index, set_branch_rm)
            for idx in remaining_edges_idx
                fbus = branch_data.node1[idx]
                tbus = branch_data.node2[idx]
                graph.add_edge(fbus, tbus)
            end
            #-----------------------------------
            # Select candidate edges to be removed
            candi_rm = Vector{Int64}()
            for branch_idx in branch_in_path
                copy_graph = copy_nx_graph(graph)
                copy_graph.remove_edge(branch_data.node1[branch_idx], branch_data.node2[branch_idx])
                if nx.is_connected(copy_graph)
                    push!(candi_rm, branch_idx)
                end
            end
            #-------------------------------------------------------------------------------------------------------
            
            if length(candi_rm) == 0
                break
            end
            ...
            if !feasibility_found # if removing one edge in the path can not result in feasibility
                ...
                j1_idx = candi_rm[ ranked_candi_rm_idx[1] ]
                new_rm_branch = union(j1_idx, set_branch_rm)
                new_branch_data = branch_data[setdiff(1:end,new_rm_branch),:]
                ...
                # Select candidate edges to be removed
                #------------------------------------------------------------------------------------
                candi_rm_IIS = Vector{Int64}()
                for branch_idx in edges_for_IIS
                    copy_graph = copy_nx_graph(graph)
                    copy_graph.remove_edge(branch_data.node1[j1_idx], branch_data.node2[j1_idx])
                    copy_graph.remove_edge(branch_data.node1[branch_idx], branch_data.node2[branch_idx])
                    if nx.is_connected(copy_graph)
                        push!(candi_rm_IIS, branch_idx)
                    end
                end
                #-----------------------------------------------------------------------------------

                if length(candi_rm_IIS) == 0
                    break
                end
                ...
                if power_flow == "none" # can not rank the edges from existing OPF solution
                    ...
                    ranked_edges_for_IIS = candi_rm_IIS[ranked_idx]
                    #-----------------------------------------------------------------
                    restore_feasibility = false
...
function compute_ksp_k_val(network,merge,load_factors,idx_load_factors,instances,branch_data,gen_data,ksp_paths,max_pathS,max_edgeS)
    ...
    for l in idx_load_factors
        for i in instances
            ...
            for p in 1:num_max_path, j in 1:num_max_edge
                ...
                k_val = ksp_for_DC_OTS(new_bus_data, branch_data, gen_data, ksp_paths, max_path, max_edge)
            ...
            k_val_df = DataFrame(k_val_mx, :auto)
            ...
            if network == 1
                output_file = "results_118/new_ksp_k_values/load_$(load_factors[l])_percent_instance_$(i).csv"
            elseif network == 2
                output_file = "results_300/new_ksp_k_values/load_$(load_factors[l])_percent_instance_$(i).csv"
            end
    ...
    if network == 1
        time_file = "results_118/new_ksp_time.csv"
    elseif network == 2
        time_file = "results_300/new_ksp_time.csv"
    end
    if merge
